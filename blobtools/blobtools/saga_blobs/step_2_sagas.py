...
from blobtools.config import COMPLETED_SAGA_QUEUE_NAME, NEW_SAGA_QUEUE_NAME, SAGA_CONTAINER_NAME, \
    STORAGE_ACCOUNT_CONNECTION_STRING

SERVICE_NAME = "step_2_sagas"
BATCH_SIZE = 10
    ...
    try:  # for the severe stuff
        while True:
            ...
            async for message in queue_client.receive_messages(max_messages=BATCH_SIZE, visibility_timeout=30):
                ...
                if blob_name.startswith('inventory'):
                    print("ignoring inventory stuff")
                ...
                try:
                    ...
                    # parent_span.add_event(f"Blob already leased: {blob_location}",
                    #                       attributes={"blob_location": blob_location})
                ...
                if saga.status == SagaStatus.completed:
                    pass
                    # parent_span.add_event(f"This blob is already complete: {saga}",
                    #                       attributes={"saga": saga.model_dump_json()})
                    ...
                    if not other_blob_exists:
                        ...
                        # parent_span.add_event(f"Other blob {other_blob_location} does not exist",
                        #                       attributes={"other_blob_location": other_blob_location})
                        ...
                        try:
                            ...
                            # parent_span.add_event(f"Blob already leased: {other_blob_location}",
                            #                       attributes={"other_blob_location": other_blob_location})
                        ...
                        if other_saga.status == SagaStatus.completed:
                            ...
                            # PRODUCE THE RESULT!
                            saga_result = SagaResult(sagas=[saga, other_saga], order_location=blob_location,
                                                     payment_location=other_blob_location)
                            # parent_span.add_event(f"Produced result: {saga_result}",
                            #                       attributes={"saga_result": saga_result.model_dump_json()})
