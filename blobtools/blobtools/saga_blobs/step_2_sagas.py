        ...
        while True:
            with trace.get_tracer(__name__).start_as_current_span(f"message-loop-{loop_metric}") as parent_span:
                ...
                async for message in queue_client.receive_messages(max_messages=BATCH_SIZE):
                    ...
                    # has active lease?
                    blob_lease = await blob_client.acquire_lease(LEASE_DURATION) if ENABLE_LEASING else None
                    ...
                    if saga.status == SagaStatus.completed:
                        ...
                        if not other_blob_exists:
                            ...
                            # get lease
                            other_blob_lease = await other_blob_client.acquire_lease(LEASE_DURATION) if ENABLE_LEASING else None
                            # update tags
